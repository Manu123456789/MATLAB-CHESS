...
class SessionDialog(QDialog):
    ...
    def _on_mode_changed(self, _idx: int) -> None:
        ...
        if mode == MODE_LOCAL:
            ...
            self.color_combo.setEnabled(True)
            self.help_label.setText(
                "Opens an existing game file. Choose the color you want to control. "
                "This lets either player rejoin an in-progress game even if the "
                "original host window was closed. Clock settings are read from the file."
            )
        elif mode == MODE_BOT:
            self.color_combo.setEnabled(True)
            self.help_label.setText(
    ...
    def _on_accept(self) -> None:
        ...
        if mode in (MODE_HOST, MODE_JOIN, MODE_SPECTATE):
            if not path:
                ...
                return
            if mode in (MODE_JOIN, MODE_SPECTATE) and not os.path.exists(path):
                QMessageBox.warning(
                    self, "File not found",
