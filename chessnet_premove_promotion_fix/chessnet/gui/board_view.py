...
from PySide6.QtGui import QPainter, QColor, QFont, QMouseEvent, QPen, QPolygonF
...
class BoardView(QWidget):
    ...
    def __init__(self, game: Game, orientation: chess.Color = chess.WHITE, parent=None):
        ...
        # Local board-annotation arrows. These are paint-only, never serialized,
        # and are cleared by the next ordinary left click. Right-click drag
        # creates persistent arrows like common chess analysis boards.
        self.annotation_arrows: list[tuple[int, int]] = []
    ...
    def mousePressEvent(self, e: QMouseEvent) -> None:
        ...
        if self.annotation_arrows or self._arrow_drag_from is not None:
            self.clear_annotation_arrows(update=False)
    ...
    def _begin_annotation_arrow(self, sq: int | None) -> None:
        if sq is None:
            ...
            self._arrow_drag_to = None
            return
        # Start only from an occupied square. This matches the requested
        # "right click on pieces" behavior while still allowing arrows to target
        # empty or occupied destination squares.
        if self._board_for_painting().piece_at(sq) is None:
            self._arrow_drag_from = None
            self._arrow_drag_to = None
            return
    ...
    def _finish_annotation_arrow(self, sq: int | None) -> None:
        ...
        self._arrow_drag_from = None
        self._arrow_drag_to = None
        if from_sq is None or sq is None or sq == from_sq:
            self.update()
            return
        arrow = (from_sq, sq)
        if arrow in self.annotation_arrows:
            ...
            self.annotation_arrows.append(arrow)
        self.update()
    ...
    def _handle_premove_click(self, sq: int) -> None:
        ...
        board = self._candidate_board_for_premove()
        piece = board.piece_at(sq)
        if piece is not None and piece.color == self.premove_color and sq not in self.premove_targets:
            self.premove_selected = sq
            self.premove_targets = self._premove_targets_from(sq)
            self.update()
            return
    ...
    def _premove_targets_from(self, sq: int) -> set[int]:
        ...
        # onto if the opponent captures that friendly piece first.
    ...
    def _make_premove(self, from_sq: int, to_sq: int) -> chess.Move | None:
        ...
        if candidates:
            # Promotion premoves are deliberately stored WITHOUT a promotion
            # piece. The user should choose Q/R/B/N only after the opponent has
            # moved and the premove is actually about to fire. This keeps the
            # premove preview natural: the pawn visibly reaches the back rank,
            # but ChessNet does not guess the promotion piece ahead of time.
            if any(m.promotion is not None for m in candidates):
                return chess.Move(from_sq, to_sq)
            return candidates[0]
        if self._piece_can_reach_own_occupied_premove_target(board, from_sq, to_sq):
            return chess.Move(from_sq, to_sq)
        return None

    def _is_premove_promotion_intent(self, board: chess.Board, move: chess.Move) -> bool:
        """Return True when a queued premove is a pawn move to the back rank.

        Queued promotion premoves intentionally omit the promotion piece so the
        main window can ask for Q/R/B/N only when the premove is ready to fire.
        """
    ...
    def _apply_preview_move(self, board: chess.Board, move: chess.Move) -> bool:
        ...
        Normal premoves are pushed legally. Conditional recapture premoves may
        move onto a square currently occupied by one of our own pieces, so they
        are applied as a paint-only board edit. The real firing path still uses
        python-chess legal_moves and will only write the move if the opponent's
        reply made it legal.
        """
        board.turn = self.premove_color
        ...
        if self._is_premove_promotion_intent(board, move):
            # A promotion premove has no promotion piece yet, so python-chess
            # quite correctly will not consider it legal. For the local preview,
            # show the pawn on the back rank and defer the actual Q/R/B/N choice
            # until the premove fires in MainWindow._try_fire_premove().
            piece = board.piece_at(move.from_square)
            if piece is None:
                return False
            target = board.piece_at(move.to_square)
            if target is not None and target.color == piece.color and not self._piece_can_reach_own_occupied_premove_target(board, move.from_square, move.to_square):
                return False
            board.remove_piece_at(move.from_square)
            board.set_piece_at(move.to_square, chess.Piece(chess.PAWN, piece.color))
            board.clear_stack()
            board.turn = self.premove_color
            return True

        if not self._piece_can_reach_own_occupied_premove_target(board, move.from_square, move.to_square):
            return False
        piece = board.piece_at(move.from_square)
        if piece is None:
            return False
        board.remove_piece_at(move.from_square)
        # Paint the selected piece on the recapture square. This intentionally
        # hides the friendly piece that is expected to be captured by the
        # opponent before the premove can fire.
        board.set_piece_at(move.to_square, chess.Piece(piece.piece_type, piece.color))
        board.clear_stack()
        board.turn = self.premove_color
        return True
    ...
    def paintEvent(self, _ev) -> None:
        ...
        for from_sq, to_sq in self.annotation_arrows:
            self._draw_annotation_arrow(p, from_sq, to_sq)
        if self._arrow_drag_from is not None and self._arrow_drag_to is not None \
                and self._arrow_drag_to != self._arrow_drag_from:
            self._draw_annotation_arrow(p, self._arrow_drag_from, self._arrow_drag_to, preview=True)
    ...
    def _draw_annotation_arrow(self,
                               p: QPainter,
                               from_sq: int,
                               to_sq: int,
                               *,
                               preview: bool = False) -> None:
        start = self._center_for(from_sq)
        end = self._center_for(to_sq)
        dx = end.x() - start.x()
        dy = end.y() - start.y()
        ...
        ux = dx / length
        uy = dy / length
        # Shorten the arrow so the head lands inside the destination square,
        # leaving the destination piece/target still recognizable.
        start_margin = TILE_PX * 0.18
        end_margin = TILE_PX * 0.25
        line_start = QPointF(start.x() + ux * start_margin,
                             start.y() + uy * start_margin)
        tip = QPointF(end.x() - ux * end_margin,
                      end.y() - uy * end_margin)
        ...
        p.drawLine(line_start, tip)

        # Arrowhead is an isosceles triangle oriented along the drag vector.
