...
from PySide6.QtWidgets import (
    ...
    QPushButton, QStatusBar, QMessageBox, QDialog, QApplication,
)
...
from .best_moves import BestMovesWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self,
        ...
        self._was_waiting_for_opponent_before_refresh = False
        ...
        self.attention_label = QLabel("Opponent moved. Click or focus this window to clear.")
        self.attention_label.setVisible(False)
        self.attention_label.setStyleSheet(
            "background: #fff2b2; border: 1px solid #c28a00; "
        ...
        self.best_moves_widget = BestMovesWidget()
        self.best_moves_widget.setVisible(self.evaluation_enabled)
        side_panel = QVBoxLayout()
        side_panel.setContentsMargins(0, 0, 0, 0)
        side_panel.setSpacing(8)
        side_panel.addWidget(self.graveyard)
        side_panel.addWidget(self.best_moves_widget)
        ...
        board_row.addWidget(side_panel_widget)
        board_row.addStretch(1)
        ...
        self.resign_btn.setVisible(not self.spectator)
        row.addWidget(self.resign_btn)
    ...
    def focusInEvent(self, event) -> None:
        self._clear_attention_notice()
        super().focusInEvent(event)

    def changeEvent(self, event) -> None:
        # If the user restores the window after a taskbar alert, clear the
        # small in-window notice as soon as the window becomes active again.
    ...
    def _promotion_candidates_for_premove(self, move: chess.Move) -> list[chess.Move]:
        """Legal promotion moves matching a queued no-piece premove.

        Premove promotion is intentionally deferred: the queued move records only
        from/to, then once the opponent has moved and this premove can actually
        fire, the player chooses Q/R/B/N.
        """
        if move.promotion is not None:
            return []
        return [m for m in self.game.board.legal_moves
                if m.from_square == move.from_square
                and m.to_square == move.to_square
                and m.promotion is not None]

    def _resolve_premove_promotion(self, move: chess.Move) -> chess.Move | None:
        candidates = self._promotion_candidates_for_premove(move)
        if not candidates:
            return move if move in self.game.board.legal_moves else None

        dlg = PromotionDialog(self.game.board.turn, parent=self)
        if dlg.exec() != QDialog.Accepted or dlg.choice is None:
            self.statusBar().showMessage("Promotion premove cancelled.", 3000)
            return None

        promoted = chess.Move(move.from_square, move.to_square, promotion=dlg.choice)
        if promoted not in candidates:
            self.statusBar().showMessage("Discarded invalid promotion premove.", 3000)
            return None
        return promoted
    ...
    def _try_fire_premove(self) -> None:
        ...
        while self.board_view.premove_queue and self.netgame.my_turn:
            ...
            resolved_move = self._resolve_premove_promotion(move)
            if resolved_move is None:
                continue

            if resolved_move not in self.game.board.legal_moves:
                self.statusBar().showMessage(
                    f"Discarded illegal premove {move.uci()}.", 3000
                )
                continue
            self.game.push(resolved_move)
            self.board_view.update()
            self.statusBar().showMessage(
    ...
    def _on_engine_options(self) -> None:
        ...
        if dlg.exec() == QDialog.Accepted:
            ...
            self.best_moves_widget.setVisible(True)
            self._refresh_engine_buttons()
            self._schedule_engine_evaluation(force=True)
            QTimer.singleShot(0, self._schedule_bot_move_if_needed)
    ...
    def _refresh_engine_buttons(self) -> None:
        ...
        if hasattr(self, 'advantage_bar'):
            self.advantage_bar.setVisible(self.evaluation_enabled)
        if hasattr(self, 'best_moves_widget'):
            self.best_moves_widget.setVisible(self.evaluation_enabled)
    ...
    def _schedule_engine_evaluation(self, *, force: bool = False) -> None:
        ...
        if hasattr(self, 'best_moves_widget'):
            self.best_moves_widget.set_thinking()
        self._refresh_engine_buttons()
    ...
    def _on_engine_eval_result(self, result: object, request_id: str) -> None:
        ...
        if analysis is None:
            ...
            if hasattr(self, 'best_moves_widget'):
                self.best_moves_widget.clear("No engine analysis available.")
            return
        ...
        if hasattr(self, 'best_moves_widget'):
            self.best_moves_widget.set_analysis(analysis, current_board, max_moves=5)
    ...
    def _on_engine_eval_failed(self, message: str, request_id: str) -> None:
        ...
        if hasattr(self, 'best_moves_widget'):
            self.best_moves_widget.clear("Engine evaluation failed.")
    ...
    def _refresh_from_file(self, *, force: bool = False) -> None:
        ...
        try:
            self._was_waiting_for_opponent_before_refresh = (
                bool(self.is_network and not self.spectator and self.netgame is not None)
                and not self.netgame.my_turn
            )
            try:
            ...
            notify_attention = (
                not same_board
                and not new_session
                and self._was_waiting_for_opponent_before_refresh
                and self.netgame is not None
                and self.netgame.my_turn
            )
            self._apply_snapshot(
                snap,
                board_updated=not same_board,
    ...
    def _apply_snapshot(self, snap: GameSnapshot, *, board_updated: bool, new_session: bool = False, notify_attention: bool = False) -> None:
        ...
        if (notify_attention and not self._analysis_mode
                and self.netgame is not None and self.netgame.my_turn):
            self._notify_opponent_move_arrived()


    # --- attention / opponent-move notification ----------------------------

    def _notify_opponent_move_arrived(self) -> None:
        """Draw attention after an opponent move arrives from the shared file.

        On Windows, QApplication.alert maps to the taskbar flash/orange
        attention behavior when the window is minimized or not active. When the
        window is visible but not focused, we also show a small banner inside
        the app; focusing the window clears it. If the window is already active,
        no extra notification is needed.
        """
        if self.spectator or self._is_terminal_status(self._effective_status()):
            return
        QApplication.alert(self, 0)
        if not self.isActiveWindow():
            self.attention_label.setText("Opponent moved. It is your turn.")
            self.attention_label.setVisible(True)
        else:
            self._clear_attention_notice()

    def _clear_attention_notice(self) -> None:
    ...
    def _maybe_start_clock_if_ready(self) -> bool:
        ...
        if self._is_terminal_status(self._effective_status()):
            return False
        # Timed games intentionally do not start burning White's clock while
        # White is thinking on the first move. The clock arms only after the
        # first legal move has been made, at which point Black's clock starts.
        if len(self.game.history) == 0:
            self.timer_state.active_color = 'w'
    ...
    def _refresh_pause_button(self) -> None:
        ...
        self.pause_btn.setEnabled(
            self.timer_state.enabled
            and not self.timer_state.expired_color
            and (self.timer_state.running or self.timer_state.paused)
        )
    ...
    def _effective_status(self) -> str:
        # Snapshot-level terminal states are not represented by python-chess.
        # Keep them authoritative so network refreshes, resignation, and clock
        # flags continue to lock input and show the correct game-over reason.
        if self.snapshot_status == 'timeout' or self.snapshot_status in ('resign_w', 'resign_b'):
    ...
    def _status_for_snapshot(self) -> str:
        if self.timer_state.enabled and self.timer_state.expired_color:
            return 'timeout'
        if self.snapshot_status in ('resign_w', 'resign_b'):
            return self.snapshot_status
        return self.game.status()
    ...
    def _result_for_snapshot(self) -> Optional[str]:
        ...
        if self.snapshot_status == 'resign_w':
            return '0-1'
        if self.snapshot_status == 'resign_b':
            return '1-0'
        return None

    def _is_terminal_status(self, status: str) -> bool:
        return (status == 'timeout' or status in ('resign_w', 'resign_b')
                or status == 'checkmate' or status == 'stalemate' or status.startswith('draw_'))
    ...
    def _refresh_status(self) -> None:
        ...
        if status in ('resign_w', 'resign_b'):
            resigned = 'w' if status == 'resign_w' else 'b'
            loser = _color_name(resigned)
            winner = _color_name('b' if resigned == 'w' else 'w')
            self.status_label.setText(f"{loser} resigned -- {winner} wins.")
            return
    ...
    def _resigning_color_code(self) -> str:
        """Return the color that should be resigned by this window.

        Network mode resigns the local player's color. Stockfish mode resigns
        the human color even if the engine is currently thinking. Local
        hot-seat resigns the side to move, matching over-the-board behavior.
        """
        if self.is_network and self.netgame is not None:
            return self.netgame.my_color
        if self.bot_color is not None:
            human = chess.BLACK if self.bot_color == chess.WHITE else chess.WHITE
            return _color_code(human)
        return _color_code(self.game.turn)

    def _result_for_resignation(self, resigned_color: str) -> str:
        return '0-1' if resigned_color == 'w' else '1-0'

    def _refresh_resign_button(self) -> None:
        if not hasattr(self, 'resign_btn'):
            return
        if self.spectator:
            self.resign_btn.setVisible(False)
            return
        enabled = (not self._analysis_mode
                   and self._view_index is None
                   and not self._replay_in_progress
                   and not self._is_terminal_status(self._effective_status()))
        self.resign_btn.setEnabled(enabled)

    def _on_resign(self) -> None:
        if self.spectator or self._analysis_mode or self._view_index is not None:
            return
        if self._is_terminal_status(self._effective_status()):
            return

        resigned = self._resigning_color_code()
        loser = _color_name(resigned)
        winner = _color_name('b' if resigned == 'w' else 'w')
        r = QMessageBox.question(
            self,
            "Resign Game",
            f"Resign as {loser}? {winner} will win the game.",
        ...
        self.snapshot_status = f"resign_{resigned}"
        if self.timer_state.enabled:
            self.timer_state.stop(now_iso())
        ...
        if self.is_network:
            ...
            snap = GameSnapshot.from_game(
                ...
                result=self._result_for_resignation(resigned),
            )
            try:
                saved = self.netgame.save_snapshot(snap)
                self.snapshot_status = saved.status
                ...
                QMessageBox.warning(
                    self,
                    "Resignation not saved",
                    "The shared file changed before the resignation could be saved. "
                ...
                QMessageBox.warning(
                    self,
                    "Resignation not saved",
                    f"Could not write the resignation to the shared file:\n\n{e}",
                )
                return
