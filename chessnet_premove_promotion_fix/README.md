...
## Status: Milestone 6 -- Stockfish bot, spectating, clocks, premoves, replay, and analysis
...
- **Stockfish / UCI engine integration**:
  - Startup mode: **Play against Stockfish**. Choose your human color, optional clock, Stockfish executable, ELO/skill, depth/movetime/nodes, threads, hash, MultiPV, and extra UCI options.
  - Engine searches run in worker threads so a slow engine does not freeze the Qt board.
  - The selected engine path/settings are saved in `~/.chessnet_stockfish.json` and reused by later sessions.
  - Bot moves are validated against `python-chess` before being applied.
- **Spectate with Stockfish evaluation** -- open an existing shared game read-only and show a left-side advantage bar that evaluates every refreshed position.
- **Analyze Position** button -- manually refreshes the engine evaluation on the current/analysis board.
...
- **35 unit tests pass** covering rules, serialization, timers, premove preview behavior, persistent history, analysis variations, replay behavior, Stockfish UCI parsing/wrapping, and the network layer.
...
| 6 | Done: Stockfish bot play, engine options, spectator evaluation bar |
| 7 | Captured-piece graveyard panel |
| 8 | PyInstaller bundle (single `chess.exe`, no Python install needed) |
...
chessnet/
  ...
  engine.py            # Stockfish/UCI wrapper, option model, eval parsing
  gui/
    main_window.py     # QMainWindow, wires together model + netgame + watcher + engine workers
    advantage_bar.py   # Stockfish score bar shown next to the board
    engine_options_dialog.py # Stockfish path/UCI/search option editor
    engine_workers.py  # QThread bridge for blocking UCI requests
...
tests/
  ...
  test_engine.py           # 4 tests: engine options, score parsing, mock UCI wrapper
main.py
```
...
## One-Click Windows App / Executable

For normal users, the easiest way to run ChessNet is to use the packaged Windows executable:

```text
ChessNet.exe
```

If you downloaded a release package, open:

```text
ChessNet.exe
```

No terminal commands are required for the packaged executable.

### Build the Executable from Source

Developers can build the executable locally on Windows by double-clicking:
...
1. Create a local `.venv` virtual environment if needed.
2. Install `requirements.txt`.
3. Install `requirements-build.txt`.
4. Run PyInstaller using `ChessNet.spec`.
5. Create the executable and copy it directly to your Desktop:

```text
%USERPROFILE%\Desktop\ChessNet.exe
```

A backup copy is also kept at:

```text
dist\ChessNet.exe
```

### First-Run Source Launcher

If you are not building an executable but still want a double-click launcher for source mode, double-click:

```text
run_chessnet.bat
```

This creates the virtual environment and installs requirements automatically on first run, then launches the app.

### Stockfish Note

Stockfish is not bundled with ChessNet. Download Stockfish separately and select the Stockfish `.exe` from the ChessNet GUI when using bot or analysis features.
