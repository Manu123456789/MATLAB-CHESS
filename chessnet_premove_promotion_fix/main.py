...
def _host_window(file_path: str, host_color: str, timer_enabled: bool = False, white_seconds: int = 600, black_seconds: int = 600, increment_seconds: int = 0) -> MainWindow | None:
...
def _join_window(file_path: str, my_color: str | None = None) -> MainWindow | None:
    ...
    Earlier versions always joined as the color opposite ``hostColor``. That
    made it impossible to reopen the host side after the original host closed
    their window. The startup dialog now lets the user explicitly choose White
    or Black when joining an in-progress game.
    """
    file_path = os.path.abspath(file_path)
    try:
