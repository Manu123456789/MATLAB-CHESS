...
class MainWindow(QMainWindow):
    ...
    def _try_fire_premove(self) -> None:
        ...
        while self.board_view.premove_queue and self.netgame.my_turn:
            ...
            if move not in self.game.board.legal_moves:
                promotion_move = self._resolve_premove_promotion(move)
                if promotion_move is None:
    ...
    def _resolve_premove_promotion(self, move: chess.Move) -> chess.Move | None:
        """Return a legal promoted version of a queued premove, if needed.

        Premoves intentionally store a pawn-to-back-rank move without a
        promotion piece so the user does not have to choose while it is still
        the opponent's turn. Once the premove is about to fire, we ask for the
        promotion piece and convert the queued move to the selected legal
        promotion move. Cancelling the dialog cancels only that premove.
        """
        piece = self.game.board.piece_at(move.from_square)
        if piece is None or piece.piece_type != chess.PAWN:
            return None
        final_rank = 7 if piece.color == chess.WHITE else 0
        if chess.square_rank(move.to_square) != final_rank:
            return None
        legal_promotions = {
            m.promotion for m in self.game.board.legal_moves
        ...
        if not legal_promotions:
            return None
        dlg = PromotionDialog(piece.color, parent=self)
        if dlg.exec() != QDialog.Accepted or dlg.choice is None:
            return None
        promoted = chess.Move(move.from_square, move.to_square, promotion=dlg.choice)
        if promoted not in self.game.board.legal_moves:
            return None
        return promoted
