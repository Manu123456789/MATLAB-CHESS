...
class TimerState:
    ...
    white_increment_sec: int = 0
    black_increment_sec: int = 0
    running: bool = False
    paused: bool = False
    paused_by: str = ''
    active_color: str = 'w'
    ...
    def from_seconds(cls,
                     ...
                     *,
                     ...
                     white_increment_seconds: Optional[int] = None,
                     black_increment_seconds: Optional[int] = None,
                     opened_color: str = '',
        ...
        inc = max(0, int(round(increment_seconds or 0)))
        white_inc = inc if white_increment_seconds is None else max(0, int(round(white_increment_seconds)))
        black_inc = inc if black_increment_seconds is None else max(0, int(round(black_increment_seconds)))
        t = cls(
            ...
            white_increment_sec=white_inc,
            black_increment_sec=black_inc,
            active_color='w',
        )
        if local:
            t.white_opened = True
    ...
    def from_dict(cls, d: Optional[dict]) -> 'TimerState':
        ...
        default_inc = int(d.get('incrementSeconds', d.get('increment_seconds', 0)) or 0)
        white_inc = int(d.get('whiteIncrementSec', d.get('white_increment_sec', default_inc)) or 0)
        black_inc = int(d.get('blackIncrementSec', d.get('black_increment_sec', default_inc)) or 0)
        return cls(
            ...
            white_increment_sec=max(0, white_inc),
            black_increment_sec=max(0, black_inc),
    ...
    def normalized(self) -> 'TimerState':
        ...
        self.white_increment_sec = max(0, int(round(self.white_increment_sec)))
        self.black_increment_sec = max(0, int(round(self.black_increment_sec)))
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'whiteIncrementSec': self.white_increment_sec,
            'blackIncrementSec': self.black_increment_sec,
            'incrementSeconds': self.white_increment_sec if self.white_increment_sec == self.black_increment_sec else 0,
    ...
    def increment_for(self, color: str) -> int:
        return self.white_increment_sec if color == 'w' else self.black_increment_sec
    ...
    def switch_after_move(self, mover_color: str, next_color: str, now: Optional[str] = None) -> None:
        ...
        if self.expired_color:
            return
        inc = self.increment_for('w' if mover_color == 'w' else 'b')
        if inc > 0:
            if mover_color == 'w':
                self.white_remaining_sec += inc
            else:
                self.black_remaining_sec += inc
...
class GameSnapshot:
    ...
    color_assignment: dict = field(default_factory=dict)
    ...
    def from_game(cls,
        ...
        return cls(
            ...
            players=dict(base.players or {}),
            setup=dict(base.setup or {}),
            color_assignment=dict(base.color_assignment or {}),
        )
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'players': dict(self.players or {}),
            'setup': dict(self.setup or {}),
            'colorAssignment': dict(self.color_assignment or {}),
        }
    ...
    def from_dict(cls, d: dict) -> 'GameSnapshot':
        ...
        return cls(
            ...
            players=dict(d.get('players') or {}),
            setup=dict(d.get('setup') or {}),
            color_assignment=dict(d.get('colorAssignment') or d.get('color_assignment') or {}),
        )
