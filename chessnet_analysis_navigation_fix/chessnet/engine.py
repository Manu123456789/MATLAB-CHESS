...
class EngineSettings:
    ...
    def for_analysis(cls, base: 'EngineSettings') -> 'EngineSettings':
        ...
        s.search_timeout_sec = max(6.0, min(20.0, float(s.search_timeout_sec)))
        return s
    ...
    def normalized(self) -> 'EngineSettings':
        ...
        self.search_timeout_sec = max(1.0, float(12.0 if self.search_timeout_sec is None else self.search_timeout_sec))
...
class StockfishEngine:
    ...
    def _effective_search_timeout(self, board: chess.Board, timer_state=None) -> float:
        ...
        """
        s = self.settings
        cap = max(1.0, float(s.search_timeout_sec))
        if s.use_clock and timer_state is not None and getattr(timer_state, 'enabled', False):
            try:
                ...
                remaining = 5.0
            return max(1.5, min(cap, remaining * 0.50 + 2.0))
        if s.search_mode == 'movetime':
            return max(1.5, min(cap, float(s.movetime_ms) / 1000.0 + 2.0))
        if s.search_mode == 'depth':
            return max(2.0, min(cap, 2.0 + 0.75 * float(s.depth)))
        if s.search_mode == 'nodes':
            return max(2.0, min(cap, 2.0 + min(8.0, float(s.nodes) / 1_000_000.0)))
        return cap
    ...
    def _wait_for(self, token: str, timeout_sec: float) -> list[str]:
        ...
        if token == 'bestmove':
            ...
            try:
                ...
                pass
        raise TimeoutError(f"Timed out waiting for Stockfish token {token!r} after {timeout_sec:.2f} seconds.")
