...
def test_permission_error_on_replace_falls_back_to_direct_write():
    """Some Windows/cloud folders deny replace/delete permissions even when
    normal file writing is allowed. NetGame should fall back to direct rewrite
    instead of failing the move."""
    import chessnet.netgame as netgame_mod

    path = _tmppath()
    original_replace = netgame_mod.os.replace
    try:
        snap = GameSnapshot.initial('w')
        ng = NetGame(path, 'w', snap)
        ng.bootstrap(snap)
        ...
        def blocked_replace(src, dst):
            raise PermissionError("simulated Windows/cloud replace denial")

        netgame_mod.os.replace = blocked_replace
        game = ng.last_seen.to_game()
        game.push(chess.Move.from_uci('e2e4'))
        ng.save(game)

        ng2 = NetGame(path, 'b', snap)
        loaded = ng2.load(allow_rebind=True)
        assert loaded.half_move_count == 1
        assert loaded.history[-1].uci == 'e2e4'
    finally:
        netgame_mod.os.replace = original_replace
        if os.path.exists(path):
            os.remove(path)
