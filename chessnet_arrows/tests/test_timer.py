...
def test_increment_round_trip():
    timer = TimerState.from_seconds(True, 60, 60, increment_seconds=30, local=True)
    snap = GameSnapshot.initial('w', timer=timer)
    back = GameSnapshot.from_json(snap.to_json())
    assert back.timer.white_increment_sec == 30
    assert back.timer.black_increment_sec == 30
    assert back.timer.increment_for('w') == 30
    assert back.timer.increment_for('b') == 30


def test_increment_added_after_completed_move():
    timer = TimerState.from_seconds(True, 30, 120, increment_seconds=10, local=True)
    timer.start('w', '2026-05-14T12:00:00')
    timer.switch_after_move('w', 'b', '2026-05-14T12:00:05')
    assert timer.white_remaining_sec == 35
    assert timer.black_remaining_sec == 120
    assert timer.active_color == 'b'


def test_increment_not_added_after_flag_fall():
    timer = TimerState.from_seconds(True, 3, 120, increment_seconds=10, local=True)
    timer.start('w', '2026-05-14T12:00:00')
    timer.switch_after_move('w', 'b', '2026-05-14T12:00:05')
    assert timer.white_remaining_sec == 0
    assert timer.expired_color == 'w'
