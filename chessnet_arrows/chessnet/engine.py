...
class EngineSettings:
    ...
    def for_analysis(cls, base: 'EngineSettings') -> 'EngineSettings':
        ...
        s.skill_level = 20
        s.multipv = max(5, int(s.multipv))
        s.ponder = False
        # Passive evaluation should be bounded by the explicit search settings,
        # not by the live chess clock. If clock-mode leaks into analysis and a
        # side is low on time, Stockfish can return instantly or fail to search
        # because the remaining time is near/below its Move Overhead.
        s.use_clock = False
        ...
        s.search_timeout_sec = max(10.0, min(60.0, float(s.search_timeout_sec)))
        return s
    ...
    def normalized(self) -> 'EngineSettings':
        ...
        self.search_timeout_sec = max(2.0, min(300.0, float(30.0 if self.search_timeout_sec is None else self.search_timeout_sec)))
    ...
    def _fixed_budget_go_command(self) -> str:
        """Return the explicit non-clock search command.

        This is used both directly and as a safety fallback when the chess
        clock is so low that passing wtime/btime to Stockfish would make the
        engine think it has no budget after its own Move Overhead reserve.
        """
    ...
    def go_command(self, board: chess.Board, timer_state=None) -> str:
        if self.use_clock and timer_state is not None and getattr(timer_state, 'enabled', False):
            ...
            side_remaining = w if board.turn == chess.WHITE else b
            # Stockfish subtracts the UCI Move Overhead from its available
            # clock budget. If the remaining time is below/near that reserve,
            # the engine may immediately stop or return no move. In that case,
            # use the user's explicit search limit instead so the bot still
            # produces a legal move. The game clock can still flag the player
            # outside this engine call if time has actually expired.
            min_clock_budget = max(500, int(self.move_overhead_ms) + 250)
            if side_remaining <= min_clock_budget:
                return self._fixed_budget_go_command()
            return (
...
class StockfishEngine:
    ...
    def analyze(self, board: chess.Board, timer_state=None) -> EngineAnalysis:
        ...
        lines = self._wait_for('bestmove', self._effective_search_timeout(board, timer_state=timer_state))
    ...
    def _reader_loop(self) -> None:
        """Continuously read Stockfish output so timeout logic works on Windows.

        A direct ``stdout.readline()`` in the request thread can block forever on
        Windows pipes, which made the GUI appear to ignore low depth/movetime
        settings. The reader thread converts blocking pipe reads into timed queue
        reads used by ``_wait_for``.
        """
        p = self.process
        ...
        try:
            ...
            self._stdout_queue.put(f"__CHESSNET_READER_ERROR__ {e}")

    def _effective_search_timeout(self, board: chess.Board, timer_state=None) -> float:
        """Return a hard wall-clock deadline for a single UCI ``go`` command.

        Stockfish normally respects depth/movetime/nodes, but engine binaries,
        cloud folders, antivirus hooks, or an accidental expensive setting can
        still leave the GUI waiting too long. This timeout is intentionally tied
        to the requested search budget instead of blindly using the saved global
        timeout value.
        """
        s = self.settings
        cap = max(2.0, float(s.search_timeout_sec))
        if s.use_clock and timer_state is not None and getattr(timer_state, 'enabled', False):
            try:
                side = 'w' if board.turn == chess.WHITE else 'b'
                remaining = float(timer_state.remaining_for(side))
            except Exception:
                remaining = 5.0
            overhead = max(0.0, float(s.move_overhead_ms) / 1000.0)
            # This is an application watchdog, not Stockfish's Move Overhead.
            # Give the engine enough time to initialize/search/stop even for
            # very low chess-clock values.
            return max(5.0, min(cap, remaining * 0.50 + overhead + 4.0))
        if s.search_mode == 'movetime':
            return max(5.0, min(cap, float(s.movetime_ms) / 1000.0 + 4.0))
        if s.search_mode == 'depth':
            return max(8.0, min(cap, 4.0 + 1.25 * float(s.depth)))
        if s.search_mode == 'nodes':
            return max(8.0, min(cap, 4.0 + min(20.0, float(s.nodes) / 500_000.0)))
        return cap
    ...
    def _wait_for(self, token: str, timeout_sec: float) -> list[str]:
        ...
        while True:
            ...
            try:
                ...
                continue
            if line.startswith('__CHESSNET_READER_ERROR__'):
        ...
        if token == 'bestmove':
            # Hard stop the search and give Stockfish a short grace period to
            # report the best move found so far. This prevents a bot move or
            # spectate analysis from blocking the GUI for a full timeout cycle.
            try:
                self._send('stop')
            except Exception:
                pass
            ...
            try:
                ...
                pass
        raise TimeoutError(
            f"Timed out waiting for Stockfish token {token!r} after {timeout_sec:.2f} seconds. "
            "This is ChessNet's application watchdog, not Stockfish's UCI Move Overhead. "
            "Increase the Engine hard timeout in Engine Options, or switch Search limit to Move time/Depth."
        )
