classdef ChessMasters
        ...
        function this = ChessMasters(shortcut)
            ...
            if nargin >= 1 && strcmp(shortcut, 'local')
                choice = struct('mode','local','color','w','filePath','', ...
                    'stockfishPath','', 'stockfishOptions',StockfishBot.defaultOptions(), ...
            ...
            switch choice.mode
                ...
                case 'bot'
                    this = this.startBot(choice.color, choice.timerEnabled, choice.timerMinutes, ...
                        choice.stockfishPath, choice.stockfishOptions);
            end
        end
    end
    ...
    methods (Access = private)
        ...
        function this = startBot(this, humanColor, timerEnabled, timerMinutes, enginePath, stockfishOptions)
            this.chessBoardModel = ChessBoardModel();
            this.gameController  = GameController(this.chessBoardModel);
            timerState = GameState.makeTimerState(timerEnabled, timerMinutes);
            this.stockfishBot = StockfishBot(enginePath, stockfishOptions);
            try
                this.stockfishBot.start();
            catch err
                errordlg(sprintf('Could not start Stockfish:\n%s', err.message), ...
                    'Stockfish failed', 'modal');
                return;
            end
            this.chessBoardGUI = ChessBoardGUI(this.chessBoardModel, this.gameController, [], ...
                humanColor, timerState, this.stockfishBot, humanColor);
        end
