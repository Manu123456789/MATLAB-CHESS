...
class NetGame:
    ...
    def _write_atomic(self, snapshot: GameSnapshot) -> None:
        """Write a snapshot to the shared game file.

        The preferred path is atomic: write a same-directory temp file and then
        replace the target. On Windows cloud-sync folders and some network
        shares, ``os.replace`` can raise ``PermissionError`` even when the user
        is allowed to modify the file because the share denies delete/rename
        access or a sync client briefly locks the target. In that specific case
        we retry, then fall back to an in-place rewrite of the existing JSON
        file. The fallback is less atomic, but it lets valid shared-folder games
        continue instead of incorrectly telling the player they cannot write.
        """
        directory = os.path.dirname(self.file_path) or '.'
        base = os.path.basename(self.file_path)
        if not os.path.isdir(directory):
            raise FileNotFoundError(
                f"The shared game folder does not exist:\n{directory}"
            )
        ...
        try:
            self._write_temp_file(tmp_path, txt)
            self._replace_with_retry(tmp_path)
        except PermissionError as first_error:
            # Some Windows/OneDrive/SMB locations allow writing the file but not
            # replacing/deleting it. Try a direct rewrite before surfacing the
            # permission error. This keeps network games usable in restricted
            # shared folders.
            try:
                self._cleanup_temp(tmp_path)
                self._write_direct_with_retry(txt)
            except PermissionError as second_error:
                raise PermissionError(
                    "ChessNet could not update the shared game file.\n\n"
                    f"File: {self.file_path}\n\n"
                    "Windows reported a permission/lock error. Make sure the "
                    "game file is in a normal writable folder, is not marked "
                    "read-only, is not open in another program, and is fully "
                    "available locally if it is stored in OneDrive/Dropbox/Google Drive.\n\n"
                    f"Original error: {first_error}\n"
                    f"Fallback error: {second_error}"
                ) from second_error
            except OSError as second_error:
                raise OSError(
                    "ChessNet could not update the shared game file after the "
                    "atomic write was denied.\n\n"
                    f"File: {self.file_path}\n\n"
                    f"Original error: {first_error}\n"
                    f"Fallback error: {second_error}"
                ) from second_error
    ...
    def _write_temp_file(self, tmp_path: str, txt: str) -> None:
        with open(tmp_path, 'w', encoding='utf-8') as f:
            f.write(txt)
            self._flush_best_effort(f)
    ...
    def _write_direct_with_retry(self, txt: str) -> None:
        last_err: Optional[PermissionError] = None
        for attempt in range(self.READ_RETRIES):
            try:
