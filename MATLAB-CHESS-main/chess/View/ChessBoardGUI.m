classdef ChessBoardGUI < handle
    ...
    methods
        ...
        function onSquareClicked(this, btn, ~, file, rank)
            ...
            % Rewind locks all board input. Clicking Live returns to
            % the present position before you can move.
        ...
        function commitPendingMove(this)
            ...
            elseif strcmp(checkState, 'check')
                % The checker sees a brief toast so they know the move
                % landed as a check; the checked player sees it on
                % their next refresh (handled in onRefreshClicked).
                this.notifyCheck(opponentColor);
            end
            if isempty(this.netGame)
        ...
        function updateActionButtons(this)
            ...
            haveHist = haveNet && ~isempty(this.netGame.lastSeenState) ...
                       && isfield(this.netGame.lastSeenState, 'history') ...
                       && ~isempty(this.netGame.lastSeenState.history);
            haveLast = haveNet && ~isempty(this.netGame.lastSeenState) ...
                       && isfield(this.netGame.lastSeenState, 'lastMove') ...
                       && ~isempty(this.netGame.lastSeenState.lastMove);
            % Replay is a live-only action (it just flashes the most
            % recent move). Rewind uses Prev/Next instead.
            canReplay = haveLast && ~havePending && ~inEntry ...
                        && ~this.suppressInput && ~this.isRewound();
            ...
            % Rewind controls.
            nHist = 0;
            if haveHist; nHist = numel(this.netGame.lastSeenState.history); end
        ...
        function onReplayLastMoveClicked(this)
            ...
            this.flashLastMove(this.netGame.lastSeenState);
        end
        ...
        function onLiveClicked(this)
            if isempty(this.netGame); return; end
            if isempty(this.rewindIndex); return; end
            if ~isempty(this.pendingMove); return; end
            this.rewindIndex = [];
            ...
            % rewind had already cleared them).
            this.paintAllPremoveHighlights();
            this.updateStatusBar();
            this.updateActionButtons();
        end
        ...
        function enterRewindAt(this, idx)
            ...
            state = this.netGame.lastSeenState;
            if ~isfield(state, 'history') || idx < 1 || idx > numel(state.history)
                return;
            end
            ...
            % queue and any in-progress entry. Don't do this on every
            % step or it'd fight the no-op case.
            ...
            synth.enPassantInfo = [];   % irrelevant in view-only mode
            ...
            % Undo applyToModel's setRound side-effect -- we don't
            % want live gameplay to think it's back at move N when
            % we exit rewind.
        ...
        function onShowHistoryClicked(this)
            if isempty(this.netGame); return; end
            if ~isempty(this.netGame.lastSeenState) ...
               && isfield(this.netGame.lastSeenState, 'history') ...
               && ~isempty(this.netGame.lastSeenState.history)
                this.openHistoryViewer(this.netGame.lastSeenState.history);
            end
        end
        ...
        function onRefreshClicked(this)
            ...
            if wasRewound
                ...
                if boardUpdated && strcmp(state.status, 'checkmate')
                    this.onLiveClicked();
                    this.notifyEnd();
                    this.resetAutoPoll();
                    return;
            ...
            % If the opponent just moved and put us in check, notify
            % the checked player. Only fire when the board actually
            % changed AND it's our turn (so we don't re-notify on our
            % own check state during bootstrap loads). Check for
            % checkmate first -- a terminal checkmate is handled by
            % notifyEnd below; we don't double-notify.
            didPlayCheckmate = boardUpdated && strcmp(state.status, 'checkmate');
            if boardUpdated && strcmp(state.status, 'check') && this.netGame.isMyTurn()
                this.notifyCheck(this.netGame.myColor);
            end
            if didPlayCheckmate
                this.notifyEnd();
            end
        ...
        % ---------------------------------------------------------------
        function notifyCheck(this, checkedColor)
            % Non-blocking "the X king is in check" notification.
            % Callers pass the color of the king that is CHECKED, so
            % the receiver can reuse this regardless of whose move it
            % currently is. Uses msgbox (non-modal) so the poll loop,
            % timer, and click handlers keep running -- the original
            % dialog-based popup blocked the whole figure.
            if nargin < 2 || isempty(checkedColor)
                checkedColor = this.gameController.whoPlays();
            end
            if checkedColor == 'w'
                colorName = 'White';
            else
            ...
            try
                h = msgbox(sprintf('%s king is in check!', colorName), ...
                    'Check', 'warn', 'non-modal');
                % Auto-dismiss after a few seconds so unattended boards
                % don't accumulate stacked popups.
                if ishandle(h)
                    t = timer('ExecutionMode','singleShot', 'StartDelay', 3.5, ...
                        'TimerFcn', @(obj,~) ChessBoardGUI.safeCloseMsgbox(h, obj));
                    start(t);
                end
            catch
                % Silent: a failed popup must never break gameplay.
            end
        end
